...
<?php if($fullelement->status > 0 || grantAccess(4)):?>
<div id="element" data-id="<?=$fullelement->id?>">
        ...
        <div class="btn-group pull-right">
          ...
          <ul class="dropdown-menu">
            ...
            <?php if($editRight || grantAccess(1)):?>
                    ...
                    <li>
                        ...
                        <div class="btnWrapper"><i class='icon-lock'></i> Lock Level at
                            <select name="lvl" onchange="this.form.submit();">
                                ...
                                <?php for($i = 1; $i <= 3; $i++):?>
                                <option value="<?=$i?>" <?php if($fullelement->status == $i){ echo 'selected="selected"';} ?>><?=$i?></option>
                                <?php endfor?>
                            </select>
                        </div>
                        </form>
        ...
        <div class="modal fade hide" id="confirmDeleteMe">
            <div class="modal-header">
                <a class="close" href="#" data-dismiss="modal">&times;</a>
                <h3>Delete this <?php if(!$fullelement->isDraft)echo 'Show';else echo 'Draft'?></h3>
            </div>
            <div class="modal-body">
                <p>Are you sure you want to delete this <?php if(!$fullelement->isDraft)echo 'Show';else echo 'Draft'?>?</p>
        ...
        <div id="alternativeNamesContainer">
            <?php if($fullelement->groupedNames()):?>
            <div id="alternativeNames">
                <!--<h2>Alternative Names</h2>-->
                <table>
                    <?php foreach($fullelement->groupedNames() as $season=>$names):?>
                    <tr>
                        <td>
                            <?php if($season!=-1):?>
            ...
            <div id="newAlternativeName">
                <?=form_open("xem/newAlternativeName",array('class'=>'form-inline'))?>
                        ...
                        <select name="language">
                            <?php foreach($languages->result() as $curLang):?>
                            <option value="<?=$curLang->id?>" <?php if($curLang->id == 'us'){ echo 'selected="selected"';} ?>><?=$curLang->name?></option>
                            <?php endforeach?>
                        </select>
        ...
        <div class="clear-keep-height">
            <ul id="sortable">
                <?php foreach($fullelement->sortedEntitys() as $curLocation):$absolute_number = 1;?>
                    <?php include('editShow_singleEntity.php');?>
                <?php endforeach?>
                <li class="clear"></li>
            </ul>
            <div class="clear"></div>
